...
Progress.prototype= {
    init:function (init) {
        ...
        init.lineBgColor !==undefined?
            this.lineBgColor = init.lineBgColor:this.lineBgColor='#fff';

        //绘制线的颜色
        init.lineColor !==undefined?
            this.lineColor = init.lineColor:this.lineColor='#fff';

        //绘制文字颜色
        init.textColor !==undefined?
            this.textColor = init.textColor:this.textColor='#fff';
        ...
        timer = setInterval(function(){
            ...
            var w=ctx.measureText(txt_day).width;//获取文本宽度
            var h=this.fontSize/2;
            ctx.fillStyle=this.textColor;
            ctx.fillText(txt_day,width/2-w/2,height/2);
